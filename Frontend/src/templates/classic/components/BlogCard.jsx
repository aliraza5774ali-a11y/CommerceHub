...
const BlogCard = ({ blog, large = false }) => {
  return (
    <div
      className={`soft-lift flex rounded-2xl bg-white shadow-sm overflow-hidden ${large ? "h-[400px]" : "h-[200px]"}`}
    >
      ...
      <div
        className={`w-1/2 bg-[#f8f8f8] flex flex-col justify-between ${large ? "p-8 gap-8" : "p-6 gap-4"}`}
      >
        ...
        <div className="flex flex-col gap-3">
          <h2
            className={`leading-relaxed text-black ${large ? "text-4xl font-medium tracking-[-0.01em]" : "text-xl font-medium"}`}
          >
            {blog.title}
          </h2>
          {large && (
