...
import { getTexartProduct } from "../data/products";
import { addItem } from "../../../store/slice/cartSlice";
import { openCart } from "../../../store/slice/Uislice";

export default function TexartProductDetails() {
  ...
  useEffect(() => {
    setProduct(getTexartProduct(slug));
    setQuantity(1);
  }, [slug]);
  ...
  if (!product) {
    return (
      <div className="min-h-[60vh] p-20 text-center">
        <p className="texart-display text-2xl">Shirt not found.</p>
        ...
        <div className="lg:pt-6">
          <div className="flex text-[var(--ink)]">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star key={i} size={13} className={i < product.rating ? "fill-current" : "text-[var(--line)]"} />
            ))}
          </div>
          <h1 className="texart-display mt-3 text-3xl sm:text-4xl">{product.title}</h1>
          <div className="mt-3 flex items-center gap-3">
            <span className="text-xl font-semibold">{product.price}</span>
            {product.swatches && (
              <div className="flex gap-1">
                {product.swatches.map((c) => <span key={c} className="h-4 w-4 rounded-full border border-white shadow" style={{ background: c }} />)}
              </div>
            )}
          </div>
          ...
          <div className="mt-7 flex gap-3">
            <div className="flex items-center rounded-full border border-[var(--line)] px-1">
            ...
            <button onClick={add} className="texart-btn texart-btn-solid flex-1">Add to Bag</button>
          </div>

          <dl className="mt-8 divide-y divide-[var(--line)] rounded-2xl border border-[var(--line)] px-5">
            {product.material && (
              <div className="py-4">
